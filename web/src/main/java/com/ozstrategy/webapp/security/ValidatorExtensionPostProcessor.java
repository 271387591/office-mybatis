 ...
 * existing AppFuse Spring configuration or jar distribution files.</p>
 *
 * <p>As an example consider the following Spring bean configuration:</p>
 *
 * <pre>
   &lt;bean class=&quot;com.ozstrategy.spring.ValidatorExtensionPostProcessor&quot;&gt;
 ...
 * validator extension is included in a Spring configuration file called applicationContext-foo-validation.xml, and that
 * this configuration file is added directly below WEB-INF in the Foo web project, then the normal war overlay process
 * coupled with AppFuse's configuration file auto detection will ensure that the validation extension is automatically
 * included into the application without requiring any modification of AppFuse's existing config files.</p>
 *
...
public class ValidatorExtensionPostProcessor implements BeanFactoryPostProcessor {
   ...
   */
  @Override public void postProcessBeanFactory(ConfigurableListableBeanFactory configurableListableBeanFactory) {
...
} // end class ValidatorExtensionPostProcessor
