 ...
 *
 */
Ext.define('Oz.plugins.form.ClearButton', {
   ...
   * @private
   */
  repositionClearButton: function(textfield,width,height,oldWidth,oldHeight,eOpts) {
    var clearButtonEl = this.clearButtonEl;
    if (!clearButtonEl) {
      return;
    }
...
//    clearButtonEl.dom.style.top = clearButtonPosition.top + 'px';
    ...
    this.clearButtonEl.alignTo(this.textField.inputEl,'tl',[this.textField.inputEl.getWidth() - 15,5]);
  },
  ...
  calculateClearButtonPosition: function(textField) {
    ...
    if(textField.__imgBtnPlugin){
     right += (textField.__imgBtnPlugin.imgButtonEl.getWidth() + textField.__imgBtnPlugin.paddingLeft);
    }
    return {
      right: right,
      top: top
  ...
  updateClearButtonVisibility: function() {
    ...
    if (oldVisible != newVisible) {
      if (this.animateClearButton && this.animateWithCss3 && !(this.textField.disabled || this.textField.readOnly)) {
