 ...
 */
Ext.define('Oz.form.MultiSelect', {
    extend: 'Ext.form.field.Base',

    alternateClassName: 'Oz.ux.Multiselect',
              ...
              if (me.priorityFiled) {
                var records = this.store.getRange();
								...
								if(me.etlFileLayoutMode){
									me.setRecordDefaultValue(records);
								}
            ...
            if (me.accountExportLayoutMode) {
              ...
              model.endEdit();
						...
						if (me.etlFileLayoutMode) {
							...
							model.set('format', val['format']);
							model.set('formatId', val['formatId']);
							model.set('valueLength', val['valueLength']);
							model.beginEdit();
							model.dirty = false;
							model.editing = false;
							model.modified = {};
...
//      }
      ...
      this.callParent();
		...
		setRecordDefaultValue:function(records){
			if(!records)return;
			var dataTypes  = [];
			if (etlFileLayoutColumnDataTypeMap[configurationRegister.get('curPortfolioId')]) {
				dataTypes = etlFileLayoutColumnDataTypeMap[configurationRegister.get('curPortfolioId')];
			} else if (etlFileLayoutColumnDataTypeMap[0]) {
				dataTypes = etlFileLayoutColumnDataTypeMap[0];
			}
			if(dataTypes.length <= 0 ) {return;}
			Ext.Array.each(records, function(record) {
				var dataType = record.get('dataType');
					if(dataType){
						Ext.Array.each(dataTypes,function(tempDataType){
							if (dataType.toLowerCase() == tempDataType.dataType.toLowerCase()) {
								var formats = tempDataType.formats;
								if(formats && formats.length > 0){
									record.set('formatId', formats[0].id);
									record.set('format', formats[0].format);
								}
								return false;
							}
						});
