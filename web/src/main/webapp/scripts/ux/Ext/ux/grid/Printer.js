...
Ext.define("Ext.ux.grid.Printer", {
  ...
  statics:{
    ...
    print:function (grid) {
        ...
        for (var key in item.data) {
          ...
          Ext.each(columns, function (column, col) {
            if (column.dataIndex == key) {
              ...
              convertedData['rownumberer'] = row + 1;
      ...
      var pagingToolBar = this.pagingToolBar;

      var pagingMsg = '';
      if (pagingToolBar) {
        var count = pagingToolBar.store.getCount();
        var pageData = pagingToolBar.getPageData();
        if (count === 0) {
          pagingMsg = pagingToolBar.emptyMsg;
        } else {
          pagingMsg = Ext.String.format(
      ...
      var tempHtml = '';
      try {
        tempHtml = Ext.create('Ext.XTemplate', tmep).apply(data);
      } catch (e) {
        console.log(e);
      }
      ...
      var htmlMarkup = [
        ...
        stylesInLine,
        '</style>',
        '<title>' + (grid.title ? grid.title : 'Grid Printer') + '</title>',
        '</head>',
        '<body>',
        ...
        tempHtml,
        '</table>',
        '<div style="text-align: right;font-size: 11px;font-family: arial; margin-top: 5px;">'+ pagingMsg+'</div>',
        '</body>',
        '</html>'
      ];
      ...
      try {
        html = Ext.create('Ext.XTemplate', htmlMarkup).html;
      } catch (e) {
        console.log(e);
      }
