...
(function () {
  ...
  Ext.gritter.updateText = function(id, text){
     Gritter.replaceText(id, text);
  }
  ...
  var Gritter = {
    ...
    add:function (params) {
      ...
      var lastChild = Ext.fly('gritter-notice-wrapper').last();
      if (!lastChild) {
        Ext.fly('gritter-notice-wrapper').insertHtml('beforeEnd', tmp);
      }
      else {
        lastChild.update(tmp);
      }
    ...
    replaceText: function (id, newText) {
      if (id) {
        var gritterEl = Ext.get('gritter-item-' + id);
        var wrapEl = Ext.get(gritterEl.query('.gritter-without-image'));
        if (wrapEl) {
          var textUpperEl = wrapEl.last();
          if (textUpperEl && textUpperEl.last()) {
            textUpperEl.last().update(newText);
          }
        }
        var s = Ext.get(Ext.get('gritter-item-1').query('.gritter-without-image')).last();
      }
    },
