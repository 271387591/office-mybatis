 ...
 */
CKEDITOR.dialog.add('dselect', function(editor) {

    var entityItems = new Array();
    var a;
    try{
        a = CKEDITOR.ajax.load("flowFormDataController/source");
        console.log(a);
    }catch(e){
        console.log(e);
    }
    var communicate = a.communicate;
    var i = 0;
    for(var key in communicate){
        entityItems[i] = [communicate[key],key];
        i++;
    }
//    entityItems[0] = ['我呢个去','aaaaaa'];//前面是label,后面是值.
//    entityItems[1] = ['我去','bbbbbb'];
	...
	return {
		title : editor.lang.dforms.dcommon.dSelect,
		...
		onOk : function() {
			...
			if (isInsertMode){
                ...
                element.setAttribute('xtype','entitySelect');
            }
...
//                                style : 'width:350px',
                                setup : function(name, element) {
                                    ...
                                    else if (name == 'select' && element.getAttribute('xtype') == 'entitySelect') {
...
//                                style : 'width:350px',
                ...
                },
                {
                    ...
                    items : entityItems,
                    setup : function(name,element) {
                        name == 'select' && element.getAttribute('xtype') == 'entitySelect' && this.setValue(element.getAttribute('source')
                            || '');
                    },
                    commit : function(element) {
...
//								setup : function(name, element) {
//									if (name == 'select')
//										this.setValue(element.getAttribute('txtwidth') || '');
//									if (CKEDITOR.env.webkit)
//										this.getInputElement().setStyle('width', '86px');
//								},
//								commit : function(element) {
//									if (this.getValue()){
//										element.setAttribute('txtwidth', this.getValue());
//									    element.setStyle('width',this.getValue()+'px');
//									}
//									else
//										element.removeAttribute('txtwidth');
//								}
//							},{
//									id : 'txtSize',
//									type : 'text',
...
//												items : [],
//												onChange : function() {
//													var dialog = this.getDialog(), values = dialog.getContentElement('info', 'cmbValue'), optName = dialog.getContentElement('info', 'txtOptName'), optValue = dialog.getContentElement('info', 'txtOptValue'), iIndex = getSelectedIndex(this);
//
...
//												},
//												commit : function(element) {
//													var dialog = this.getDialog(), optionsNames = getOptions(this), optionsValues = getOptions(dialog.getContentElement('info', 'cmbValue')), selectValue = dialog.getContentElement('info', 'txtValue').getValue();
//
...
//												items : [],
//												onChange : function() {
//													var dialog = this.getDialog(), names = dialog.getContentElement('info', 'cmbName'), optName = dialog.getContentElement('info', 'txtOptName'), optValue = dialog.getContentElement('info', 'txtOptValue'), iIndex = getSelectedIndex(this);
//
...
//										onClick : function() {
//											// Add new option.
//											var dialog = this.getDialog(), parentEditor = dialog.getParentEditor(), optName = dialog.getContentElement('info', 'txtOptName'), optValue = dialog.getContentElement('info', 'txtOptValue'), names = dialog.getContentElement('info', 'cmbName'), values = dialog
...
//											var dialog = this.getDialog(), optName = dialog.getContentElement('info', 'txtOptName'), optValue = dialog.getContentElement('info', 'txtOptValue'), names = dialog.getContentElement('info', 'cmbName'), values = dialog.getContentElement('info', 'cmbValue'), iIndex = getSelectedIndex(names);
//
//											if (iIndex >= 0) {
...
//										onClick : function() {
//											// Move up.
//											var dialog = this.getDialog(), names = dialog.getContentElement('info', 'cmbName'), values = dialog.getContentElement('info', 'cmbValue');
//
...
//										var dialog = this.getDialog(), values = dialog.getContentElement('info', 'cmbValue'), txtValue = dialog.getContentElement('info', 'txtValue');
...
//									onClick : function() {
//										// Delete option.
//										var dialog = this.getDialog(), names = dialog.getContentElement('info', 'cmbName'), values = dialog.getContentElement('info', 'cmbValue'), optName = dialog.getContentElement('info', 'txtOptName'), optValue = dialog.getContentElement('info', 'txtOptValue');
//
